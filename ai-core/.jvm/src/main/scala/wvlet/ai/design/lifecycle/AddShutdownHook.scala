...
object AddShutdownHook extends LifeCycleEventHandler:
  ...
  private def removeShutdownHooksFor(lifeCycleManager: LifeCycleManager): Unit = synchronized {
    shutdownHooks
    ...
    if shutdownHooks.isEmpty then {
      // AirframeLogManager.resetFinally
    }
  }

  override def beforeStart(lifeCycleManager: LifeCycleManager): Unit = synchronized {
