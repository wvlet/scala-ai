...
import wvlet.ai.util.log.LogFormatter.AppLogFormatter
import wvlet.ai.util.log.LogRotationHandler
...
object LogRotationHandler:
  ...
  private[LogRotationHandler] class StringEncoder extends EncoderBase[String]:
    override def encode(event: String): Array[Byte] = event.getBytes(StandardCharsets.UTF_8)
