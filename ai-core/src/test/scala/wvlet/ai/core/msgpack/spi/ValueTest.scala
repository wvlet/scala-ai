...
class ValueTest extends AirSpec with PropertyCheck:
  private def rankOf(mf: MessageFormat): Int =
    ...
    order.zipWithIndex.find(x => x._1 == mf).map(x => x._2).getOrElse(10000)
  ...
  test("have integer") {
    ...
    check(
      newInteger(BigInteger.valueOf(1324134134134L)),
      ValueType.INTEGER,
      "1324134134134",
      "1324134134134"
    )
  ...
  protected def jsonEq(a: String, b: String): Unit =
    ...
    def sanitize(s: String): Seq[String] =
      s.replaceAll("""[\{\}\[\]\"]""", "").split("[,:]").toIndexedSeq.sorted
