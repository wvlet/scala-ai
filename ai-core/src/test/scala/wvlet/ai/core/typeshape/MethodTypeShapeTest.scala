...
object MethodExamples:
  ...
  trait G:
    def generic[E: ClassTag](arg: String): E = new Throwable().asInstanceOf[E]
...
import MethodExamples.*

/**
  */
class MethodTypeShapeTest extends SurfaceSpec:

  test("list methods") {
    val m = TypeShape.methodsOf[A]
    debug(m.mkString("\n"))
  ...
  test("find method default parameter in trait") {
    val ms = TypeShape.methodsOf[E]
    val m  = ms.find(_.name == "hello").get
    assert(m.args.headOption.isDefined)
    val h = m.args.head
    // FIXME: Fix StaticMethodParameter in CompileTimeTypeShapeFactory for Scala 3
  ...
  test("find Any typeShape from Map[String, Any] method surface") {
    val ms = TypeShape.methodsOf[F]
    ms.find(_.name == "mapInput") match
      case Some(m) if m.args.size == 1 =>
        val arg = m.args(0)
        val p1  = arg.typeShape.typeArgs(1)
        p1.fullName shouldBe "scala.Any"
      case _ =>
        fail("F.mapInput method not found")
  }

end MethodTypeShapeTest
