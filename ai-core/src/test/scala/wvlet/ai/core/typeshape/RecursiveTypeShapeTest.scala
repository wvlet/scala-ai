...
class RecursiveSurfaceTest extends SurfaceSpec:
  ...
  test("support recursive type") {
    val c: TypeShape = TypeShape.of[Cons]
    assert(c.toString == "Cons")

    assert(c.params.length == 2)
    val h = c.params(0)
    assert(h.name == "head")
    assert(h.typeShape == Primitive.String)

    val t = c.params(1)
    assert(t.name == "tail")
    val lazyC: TypeShape = t.typeShape
  ...
  test("support generic recursive type") {
    val c: TypeShape = TypeShape.of[TypedCons[String]]
    debug(s"TypeCons[String] ${c.getClass}")
    c.toString shouldBe "TypedCons[String]"

    c.params.length shouldBe 2
    c.params(0).typeShape shouldBe Primitive.Int

    val lazyC: TypeShape = c.params(1).typeShape
    debug(s"lazyC typeShape: ${lazyC.getClass}...")
