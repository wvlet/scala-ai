...
class RecordTypeShapeTest extends AirSpec:
  test("build custom surface") {
    val p1 = RecordParameter(0, "p1", Primitive.Int)
    val p2 = RecordParameter(1, "p2", Primitive.String)
    val p3 = RecordParameter(2, "p3", OptionTypeShape(classOf[Option[Long]], Primitive.Long))
    val s  = RecordTypeShape.newTypeShape("myrecord").addParam(p1).addParam(p2).addParam(p3)
