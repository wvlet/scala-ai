...
object i3418 extends AirSpec:
  ...
  test("Support generic List of tuples") {
    val s = TypeShape.of[S]
    debug(s.params)
    s.params.size shouldBe 1
    val p1 = s.params(0)
    p1.name shouldBe "a"
    val a1 = p1.typeShape.params(0)
    a1.name shouldBe "data"
    a1.typeShape.name shouldBe "List[Tuple2[String,Int]]"
  }

  test("Support generic List of tuples with two type parameters") {
    val s = TypeShape.of[SS]
    debug(s.params)
    s.params.size shouldBe 1
    val p1 = s.params(0)
    p1.name shouldBe "a"
    val a1 = p1.typeShape.params(0)
    a1.name shouldBe "data"
    a1.typeShape.name shouldBe "List[Tuple2[String,Long]]"
  }
