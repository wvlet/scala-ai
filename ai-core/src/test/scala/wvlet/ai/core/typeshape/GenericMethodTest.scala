...
object GenericMethodTest extends AirSpec:
  ...
  test("generic method") {
    val methods = TypeShape.methodsOf[A]
  ...
  test("Methods of generic type") {
    val typeSurface = TypeShape.of[Gen[String]]
    val methods     = TypeShape.methodsOf[Gen[String]]
    val pass        = methods.find(_.name == "pass").get
    pass.returnType shouldBe TypeShape.of[String]
    ...
    unwrap.returnType shouldBe TypeShape.of[String]
  }
