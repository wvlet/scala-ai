...
private[typeshape] object CompileTimeTypeShapeFactory:

  type SurfaceMatcher = PartialFunction[Type[?], Expr[TypeShape]]

  def typeShapeOf[A](using tpe: Type[A], quotes: Quotes): Expr[TypeShape] =
    import quotes.*
    ...
    if !flags.is(Flags.JavaStatic) && flags.is(Flags.NoInits) then
      t.typeSymbol.maybeOwner match
        ...
        case s: Symbol
            if !s.isNoSymbol && s.isClassDef && !s.isPackageDef && !s.flags.is(Flags.Module) &&
...
end CompileTimeTypeShapeFactory

// TODO: Refactor CompileTimeTypeShapeFactory into smaller, more focused classes or modules
private[typeshape] class CompileTimeTypeShapeFactory[Q <: Quotes](using quotes: Q):
  import quotes.*
  import quotes.reflect.*
  import quotes.*
  import quotes.reflect.*
  ...
  private def fullTypeNameOf(t: TypeRepr): String =
    def sanitize(symbol: Symbol): String =
      val nameParts: List[String] =
        symbol.fullName.split("\\.").toList match
  ...
  private type Factory = PartialFunction[TypeRepr, Expr[TypeShape]]

  def typeShapeOf(tpe: Type[?]): Expr[TypeShape] = typeShapeOf(TypeRepr.of(using tpe))

  private var observedSurfaceCount = new AtomicInteger(0)
  private var seen                 = ListMap[TypeRepr, Int]()
  private val memo                 = scala.collection.mutable.Map[TypeRepr, Expr[TypeShape]]()
  ...
  private def typeShapeOf(t: TypeRepr, useVarRef: Boolean = true): Expr[TypeShape] =
    def buildLazyTypeShape: Expr[TypeShape] =
      '{
        LazyTypeShape(
          ${
            clsOf(t)
    ...
    if useVarRef && surfaceToVar.contains(t) then
      if lazySurface.contains(t) then
        ...
        Ref(surfaceToVar(t)).asExprOf[TypeShape]
      ...
      val generator = factory.andThen { expr =>
        if !lazySurface.contains(t) then
          ...
          // Need to cache the recursive Surface to be referenced in a LazyTypeShape
          val cacheKey =
          ...
          '{
            ...
            then
              wvlet.ai.core.typeshape.TypeShape.surfaceCache +=
                ${
                  key
                } ->
                  ${
  ...
  private def factory: Factory =
    taggedTypeFactory orElse andOrTypeFactory orElse aliasFactory orElse
      higherKindedTypeFactory orElse primitiveTypeFactory orElse arrayFactory orElse
      optionFactory orElse tupleFactory orElse javaUtilFactory orElse javaEnumFactory orElse
      exisitentialTypeFactory orElse genericTypeWithConstructorFactory orElse genericTypeFactory
  ...
  private def primitiveTypeFactory: Factory = {
    ...
    case t if t =:= TypeRepr.of[BigInt] =>
      '{
        Primitive.BigInt
      }
    case t if t =:= TypeRepr.of[java.math.BigInteger] =>
      '{
        Primitive.BigInteger
      }
  }

  private def typeNameOf(t: TypeRepr): String = t
    .typeSymbol
    .fullName
    .stripSuffix("$")
    .replaceAll("\\.package\\$", ".")
  ...
  private def isTaggedType(t: TypeRepr): Boolean = typeNameOf(t).startsWith(
    "wvlet.ai.core.typeshape.tag."
  )
  ...
  private def taggedTypeFactory: Factory = {
    case a: AppliedType if a.args.length == 2 && isTaggedType(a) =>
      '{
        TaggedTypeShape(
          ${
            typeShapeOf(a.args(0))
          },
          ${
            typeShapeOf(a.args(1))
          }
        )
      }
  }
  ...
  private def extractSymbol(t: TypeRepr): Expr[TypeShape] =
    ...
    symbolInOwner.map(_.typeRef.dealias) match
      ...
      case _ =>
        ...
        if t != dealiased then
          typeShapeOf(dealiased)
        else
          typeShapeOf(t.simplified)
  ...
  private def higherKindedTypeFactory: Factory = {
    case h: TypeLambda =>
      ...
      val inner    = typeShapeOf(h.resType)

      val len = h.paramNames.size
      val params = (0 until len).map { i =>
        h.param(i)
      }
      val args = params.map(typeShapeOf(_))
      '{
        HigherKindedTypeTypeShape(
          ${
            Expr(name)
          },
          ${
    ...
    case a: AppliedType if !a.typeSymbol.isClassDef =>
      ...
      val args     = a.args.map(typeShapeOf(_))
      // TODO support type erasure instead of using AnyRefTypeShape
      '{
        HigherKindedTypeTypeShape(
          ${
            Expr(name)
  ...
  private def elementTypeSurfaceOf(t: TypeRepr): Expr[TypeShape] =
    typeArgsOf(t).map(typeShapeOf(_)).headOption match
      case Some(expr) =>
        expr
      case None =>
        // FIXME: Is this right ?
  ...
  private def tupleFactory: Factory = {
    case t if t <:< TypeRepr.of[Product] && typeNameOf(t).startsWith("scala.Tuple") =>
      val paramTypes = typeArgsOf(t).map(typeShapeOf(_))
      '{
        new TupleTypeShape(
          ${
            clsOf(t)
  ...
  private def javaUtilFactory: Factory = {
    ...
    case t
        if t =:= TypeRepr.of[java.io.File] || t =:= TypeRepr.of[java.util.Date] ||
          t =:= TypeRepr.of[java.time.temporal.Temporal] =>
      newGenericTypeShapeOf(t)
  }

  private def isEnum(t: TypeRepr): Boolean = t
    .baseClasses
    .exists(x => x.fullName.startsWith("java.lang.Enum"))
  ...
  private def clsOf(t: TypeRepr): Expr[Class[?]] = Literal(ClassOfConstant(t))
    .asExpr
    .asInstanceOf[Expr[Class[?]]]

  private def newGenericTypeShapeOf(t: TypeRepr): Expr[TypeShape] =
    '{
      new GenericTypeShape(
        ${
          clsOf(t)
        }
      )
  ...
  private def createObjectFactoryOf(targetType: TypeRepr): Option[Expr[ObjectFactory]] =
    ...
    if flags.is(Flags.Abstract) || flags.is(Flags.Module) || hasAbstractMethods(targetType) ||
      isPathDependentType(targetType)
    then
      None
    else
              ...
              val fn =
                argListList.foldLeft[Term](cstr) { (prev, argList) =>
                  val argExtractors = argList.map { a =>
                    // args(i+1)
                    val extracted = Select
                      .unique(args, "apply")
                      .appliedTo(Literal(IntConstant(index)))
                    index += 1
        ...
        val expr =
          '{
            ObjectFactory.newFactory(
              ${
                newClassFn.asExprOf[Seq[Any] => Any]
              }
            )
          }
        expr
  ...
  end createObjectFactoryOf

  private def hasAbstractMethods(t: TypeRepr): Boolean = t
    .typeSymbol
    .methodMembers
  ...
  private def genericTypeFactory: Factory = {
    case t if t =:= TypeRepr.of[Any] =>
      '{
        Alias("Any", "scala.Any", AnyRefTypeShape)
      }
    case a: AppliedType =>
      val typeArgs = a.args.map(typeShapeOf(_))
      '{
        new GenericTypeShape(
          ${
            clsOf(a)
    ...
    case TypeBounds(a1: AppliedType, a2: AppliedType) if a1 == a2 =>
      val typeArgs = a1.args.map(typeShapeOf(_))
      '{
        new GenericTypeShape(
          ${
            clsOf(a1)
    ...
    case t if t <:< TypeRepr.of[scala.reflect.Enum] && !(t =:= TypeRepr.of[Nothing]) =>
      ...
      val newFn = Lambda(
        ...
        tpe =
          MethodType(List("cl", "s"))(
            _ => List(TypeRepr.of[Class[?]], TypeRepr.of[String]),
      ...
      '{
        EnumTypeShape(
          ...
          ${
            newFn.asExprOf[(Class[?], String) => Option[Any]]
          }
        )
      }
    case t if hasStringUnapply(t) =>
      // Build EnumTypeShape.apply code
      // EnumTypeShape(classOf[t], { (cl: Class[?], s: String) => (companion object).unapply(s).asInstanceOf[Option[Any]] }
  ...
  private def hasOptionReturnType(d: DefDef, retElementType: TypeRepr): Boolean =
    if d.returnTpt.tpe <:< TypeRepr.of[Option[?]] then
      val typeArgs = typeArgsOf(d.returnTpt.tpe)
      typeArgs.headOption match
        case Some(t) if t =:= retElementType =>
          true
        case _ =>
          false
    else
  ...
  private def hasStringUnapply(t: TypeRepr): Boolean = getStringUnapply(t).isDefined
  ...
  private def methodArgsOf(t: TypeRepr, method: Symbol): List[List[MethodArg]] =
    ...
    val paramss: List[List[Symbol]] = method
      .paramSymss
      .filter { lst =>
        // Empty arg is allowed
        lst.isEmpty ||
        // Remove type params or implicit ClassTag evidences as MethodTypeShape can't pass type parameters
        !lst.forall { x =>
          def hasFlag(flag: Flags) = x.flags.is(flag)
          def isClassTag           = x.typeRef <:< TypeRepr.of[ClassTag[?]]
          ...
          x.isTypeParam || ((hasFlag(Flags.Implicit) || hasFlag(Flags.Given)) && isClassTag)
        }
      }
  ...
  def methodsOf(t: Type[?]): Expr[Seq[MethodTypeShape]] = methodsOf(TypeRepr.of(using t))

  // To reduce the byte code size, we need to memoize the generated surface bound to a variable
  // Note: surfaceToVar is already defined as mutable.Map at line 90

  private def methodsOf(t: TypeRepr): Expr[Seq[MethodTypeShape]] =
    // Run just for collecting known surfaces. seen variable will be updated
    methodsOfInternal(t)

    // Create a var def table for replacing typeShapeOf[xxx] to __s0, __s1, ...
    ...
    seen
      ...
      .reverse
      .foreach { (tpe, order) =>
        // Update the cache so that the next call of typeShapeOf method will use the local varaible reference
        surfaceToVar += tpe ->
          Symbol.newVal(
            Symbol.spliceOwner,
            // Use alphabetically ordered variable names
            f"__s${surfaceVarCount}%03X",
            TypeRepr.of[TypeShape],
    ...
    val surfaceDefs: List[ValDef] =
      surfaceToVar
        .toSeq
        .map { case (tpe, sym) =>
          ValDef(sym, Some(typeShapeOf(tpe, useVarRef = false).asTerm.changeOwner(sym)))
        }
        .toList
    ...
    /**
      ...
      *
      * ClassMethodTypeShape( .... ) }}
      */
    val expr = Block(surfaceDefs, methodsOfInternal(t).asTerm).asExprOf[Seq[MethodTypeShape]]
  ...
  private def methodsOfInternal(targetType: TypeRepr): Expr[Seq[MethodTypeShape]] =
    if seenMethodParent.contains(targetType) then
      ...
      val methodSurfaces = localMethods
        ...
        .collect { case (m, df: DefDef) =>
          ...
          val owner        = typeShapeOf(targetType)
          ...
          val ret = typeShapeOf(returnType)
  ...
  private def isTypeParam(t: TypeRepr): Boolean =
    t match
      case TypeRef(prefix, typeName) if prefix.toString == "NoPrefix" =>
        true
      case _ =>
        false

  private def clsCast(term: Term, t: TypeRepr): Term = Select
    .unique(term, "asInstanceOf")
    .appliedToType(t)
  ...
  private def createMethodCaller(
    ...
    val methodTypeParams: List[TypeParamClause] =
      m.tree match
        case df: DefDef =>
          df.paramss
            .collect { case t: TypeParamClause =>
              t
            }
        case _ =>
          List.empty
    ...
    val lambda = Lambda(
      owner = Symbol.spliceOwner,
      tpe =
        MethodType(List("x", "args"))(
          _ => List(TypeRepr.of[Any], TypeRepr.of[Seq[Any]]),
          _ => TypeRepr.of[Any]
        ),
      rhsFn =
        (sym, params) =>
          ...
          if argList.isEmpty then
            ...
            val newExpr =
              if methodTypeParams.isEmpty then
                expr.appliedToArgss(argList)
              else
                ...
                expr.appliedToTypes(dummyTypeParams).appliedToArgss(argList)
            newExpr.changeOwner(sym)
    )
    '{
      Some(
        ${
          lambda.asExprOf[(Any, Seq[Any]) => Any]
        }
      )
    }

  end createMethodCaller
  ...
  private def localMethodsOf(t: TypeRepr): Seq[Symbol] =
    def allMethods = t
      ...
      .methodMembers
      .filter { x =>
        nonObject(x.owner) && x.isDefDef &&
        // x.isPublic &&
        x.privateWithin.isEmpty && !x.flags.is(Flags.Private) && !x.flags.is(Flags.Protected) &&
        !x.flags.is(Flags.PrivateLocal) && !x.isClassConstructor && !x.flags.is(Flags.Artifact) &&
        !x.flags.is(Flags.Synthetic) && !x.flags.is(Flags.Macro) && !x.flags.is(Flags.Implicit) &&
  ...
  private def nonObject(x: Symbol): Boolean =
    !x.flags.is(Flags.Synthetic) && !x.flags.is(Flags.Artifact) && x.fullName != "scala.Any" &&
  ...
  def surfaceFromClass(cl: Class[?]): Expr[TypeShape] =
    val name         = cl.getName
