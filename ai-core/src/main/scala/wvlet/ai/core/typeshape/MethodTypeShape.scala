...
trait MethodTypeShape extends ParameterBase:
  def mod: Int
  def owner: TypeShape
  def name: String
  def args: Seq[MethodParameter]
  def typeShape: TypeShape = returnType
  def returnType: TypeShape
...
case class StaticMethodParameter(
  ...
  override def toString: String             = s"${name}:${typeShape.name}"
