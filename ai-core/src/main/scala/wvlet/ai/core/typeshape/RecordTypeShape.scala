...
/**
  ...
  */
case class RecordTypeShape(
    name: String,
    fullName: String,
    rawType: Class[?] = classOf[Any],
    typeArgs: Seq[TypeShape] = Seq.empty,
  ...
  override def dealias: TypeShape = this

  override def isAlias: Boolean = false

  def withTypeArgs(newTypeArgs: Seq[TypeShape]): RecordTypeShape = this.copy(typeArgs = newTypeArgs)
  def withRawType(cls: Class[?]): RecordTypeShape                = this.copy(rawType = cls)
  def withParams(newParams: Seq[Parameter]): RecordTypeShape     = this.copy(params = newParams)
  def addParam(newParam: Parameter): RecordTypeShape =
    require(newParam.index == params.length, s"index must be ${params.length}: ${newParam.index}")
    this.copy(params = params :+ newParam)

  def withObjectFactory(newFactory: ObjectFactory): RecordTypeShape = this.copy(objectFactory =
    Some(newFactory)
  )

  def asRequired: RecordTypeShape  = this.copy(isRequired = true)
  def asSecret: RecordTypeShape    = this.copy(isSecret = true)
  def asPrimitive: RecordTypeShape = this.copy(isPrimitive = true)
  def asOption: RecordTypeShape    = this.copy(isOption = true)
  def asSeq: RecordTypeShape =
...
object RecordTypeShape:
  def newTypeShape(fullName: String): RecordTypeShape =
    val name = fullName.split("\\.").lastOption.getOrElse(fullName)
    RecordTypeShape(name = name, fullName = fullName)
