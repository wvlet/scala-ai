...
object PrefixedULID:
  ...
  def newPrefixedULID(prefix: String): PrefixedULID = PrefixedULID(prefix, ULID.newULID)
  ...
  def fromString(s: String): PrefixedULID =
    ...
    require(
      s.length >= 27,
      s"PrefixedULID must have 26 (ulid) + 1 (delimiter) characters: ${s} (length: ${s.length}) "
    )
