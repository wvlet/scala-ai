...
/**
  ...
  * @param formatter
  */
class ConsoleLogHandler(formatter: LogFormatter, out: PrintStream = LogEnv.defaultConsoleOutput)
    extends jl.Handler:
  override def publish(record: jl.LogRecord): Unit = out.println(formatter.format(record))
