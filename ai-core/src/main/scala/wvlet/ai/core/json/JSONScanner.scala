...
object JSONScanner:
  ...
  @inline
  final val OBJECT_START = 2

  @inline
  final val OBJECT_KEY = 3

  @inline
  final val OBJECT_END = 4

  @inline
  final val ARRAY_START = 5

  @inline
  final val ARRAY_END = 6

  @inline
  final val SEPARATOR = 7

  private[json] def scan(s: JSONSource): Unit = scan(s, new NullJSONContext(isObject = true))
...
class JSONScanner[J](private val s: JSONSource, private val handler: JSONHandler[J])
  ...
  private def scanFalse(ctx: JSONContext[J]): Unit =
    ...
    if s(cursor) == 'f' && s(cursor + 1) == 'a' && s(cursor + 2) == 'l' && s(cursor + 3) == 's' &&
      s(cursor + 4) == 'e'
    then
      cursor += 5
