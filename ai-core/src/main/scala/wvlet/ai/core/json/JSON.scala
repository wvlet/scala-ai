...
object JSON extends LogSupport:
  ...
  /**
    ...
    */
  def parse(s: String): JSONValue = parse(JSONSource.fromString(s))

  /**
    * Parse JSON object or array
    */
  def parse(s: Array[Byte]): JSONValue = parse(JSONSource.fromBytes(s))
  ...
  /**
    ...
    */
  def parseAny(s: String): JSONValue = parseAny(JSONSource.fromString(s))

  /**
    * Parse any json values including null
    */
  def parseAny(s: Array[Byte]): JSONValue = parseAny(JSONSource.fromBytes(s))
  ...
  final case class JSONObject(v: Seq[(String, JSONValue)]) extends JSONValue:
    ...
    def get(name: String): Option[JSONValue] = v.collectFirst {
  ...
  final case class JSONArray(v: IndexedSeq[JSONValue]) extends JSONValue:
    ...
    def apply(i: Int): JSONValue = v.apply(i)
