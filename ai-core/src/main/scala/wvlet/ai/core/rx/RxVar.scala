...
/**
  * A reactive variable supporting update and propagation of the updated value to the chained
  * operators
  */
class RxVar[A](private var currentValue: A) extends Rx[A] with RxVarOps[A]:
  ...
  /**
    * Updates the variable and trigger the recalculation of the subscribers currentValue =>
    * newValue. This method will tigger the update only when the value changes. When the force flag
    * is true (default: false), it will always propagate the event to the downstream subscribers.
    */
  ...
  /**
    * Stop updating this variable and send OnCompletion event to the downstream subscribers. After
    * this method is called, the behavior of the downstream subscribers is undefined for further
    * updates of this variable.
    */
  override def stop(): Unit = propagateEvent(OnCompletion)
  ...
  override def setException(e: Throwable): Unit = propagateEvent(OnError(e))
...
trait RxVarOps[A]:
  ...
  def forceSet(newValue: A): Unit = update(
    { (x: A) =>
      newValue
    },
    force = true
