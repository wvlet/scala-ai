...
/**
  ...
  */
case class ManualTicker(nanos: AtomicLong = new AtomicLong(0), autoIncrementStepNanos: Long = 0)
    extends Ticker:

  /**
    * Set the auto-increment step, which will be added after reading a value
    */
  def withIncrements(time: Long, unit: TimeUnit): ManualTicker = this.copy(autoIncrementStepNanos =
    unit.toNanos(time)
  )
...
object Ticker:
  ...
  /**
    ...
    */
  def manualTicker: ManualTicker = ManualTicker()
