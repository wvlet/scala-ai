...
object OffsetPacker:
  ...
  def packNil(cursor: WriteCursor): Unit = cursor.writeByte(NIL)

  def packBoolean(cursor: WriteCursor, v: Boolean): Unit = cursor.writeByte(
    if v then
      TRUE
    else
      FALSE
  ...
  private[msgpack] def packINT16(cursor: WriteCursor, v: Short): Unit = cursor.writeByteAndShort(
    INT16,
    v
  )

  private[msgpack] def packINT32(cursor: WriteCursor, v: Int): Unit = cursor.writeByteAndInt(
    INT32,
    v
  )

  private[msgpack] def packINT64(cursor: WriteCursor, v: Long): Unit = cursor.writeByteAndLong(
    INT64,
    v
  )

  private[msgpack] def packUINT8(cursor: WriteCursor, v: Byte): Unit = cursor.writeByteAndByte(
    UINT8,
    v
  )

  private[msgpack] def packUINT16(cursor: WriteCursor, v: Short): Unit = cursor.writeByteAndShort(
    UINT16,
    v
  )

  private[msgpack] def packUINT32(cursor: WriteCursor, v: Int): Unit = cursor.writeByteAndInt(
    UINT32,
    v
  )

  private[msgpack] def packUINT64(cursor: WriteCursor, v: Long): Unit = cursor.writeByteAndLong(
    UINT64,
    v
  )

  private[msgpack] def packFLOAT32(cursor: WriteCursor, v: Float): Unit = cursor.writeByteAndFloat(
    FLOAT32,
    v
  )

  private[msgpack] def packFLOAT64(cursor: WriteCursor, v: Double): Unit = cursor
  ...
  def packBigInteger(cursor: WriteCursor, bi: BigInteger): Unit =
    if bi.bitLength <= 63 then
      ...
      throw new IllegalArgumentException(
        s"MessagePack cannot serialize BigInteger larger than 2^64-1: ${bi}"
      )

  def packFloat(cursor: WriteCursor, v: Float): Unit = packFLOAT32(cursor, v)

  def packDouble(cursor: WriteCursor, v: Double): Unit = packFLOAT64(cursor, v)
  ...
  def packTimestampEpochSecond(cursor: WriteCursor, epochSecond: Long, nanoAdjustment: Int): Unit =
    ...
    if (sec >>> 34) == 0L then // sec can be serialized in 34 bits.
      ...
      then // sec can be serialized in 32 bits and nsec is 0.
        // use timestamp 32
  ...
  private[msgpack] def packTimestamp32(
      cursor: WriteCursor,
      sec: Int
  ): Unit = // timestamp 32 in fixext 4
  ...
  private[msgpack] def packTimestamp64(
      cursor: WriteCursor,
      data64: Long
  ): Unit = // timestamp 64 in fixext 8
  ...
  private[msgpack] def packTimestamp96(
      cursor: WriteCursor,
      sec: Long,
      nsec: Int
  ): Unit = // timestamp 96 in ext 8
  ...
  def packMap32Header(cursor: WriteCursor, mapSize: Int) = cursor.writeByteAndInt(MAP32, mapSize)
  ...
  def writePayload(cursor: WriteCursor, v: Array[Byte]): Unit = cursor.writeBytes(v)

  def writePayload(cursor: WriteCursor, v: Array[Byte], vOffset: Int, length: Int): Unit = cursor
    .writeBytes(v, vOffset, length)
