...
/**
  ...
  *   - The write methods write data to the specified position in the buffer and return the written
  *     byte length.
  *
  * If the buffer capacity is insufficient, these read/write methods throw an
  * [[InsufficientBufferException]]. If this exception is thrown, the user code should feed more
  * data, and then resume reading.
  */
trait Buffer:
  def size: Int
...
trait WriteBuffer extends Buffer:
  ...
  def writeByteAndFloat(position: Int, b: Byte, v: Float): Int = writeByteAndInt(
    position,
    b,
    Compat.floatToIntBits(v)
  )

  def writeByteAndDouble(position: Int, b: Byte, v: Double): Int = writeByteAndLong(
    position,
    b,
    Compat.doubleToLongBits(v)
  )
