...
case class WriteCursor(var buf: WriteBuffer, var position: Int):
  ...
  def ensureCapacity(size: Int): Unit = buf.ensureCapacity(position + offset, size)

  def writeByte(b: Byte): Unit = offset += buf.writeByte(position + offset, b)
  ...
  def writeInt(a: Int): Unit   = offset += buf.writeInt(position + offset, a)
  def writeLong(a: Long): Unit = offset += buf.writeLong(position + offset, a)

  def writeBytes(a: Array[Byte]): Unit = offset += buf.writeBytes(position + offset, a)
