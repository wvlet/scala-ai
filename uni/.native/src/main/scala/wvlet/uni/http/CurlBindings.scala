...
/**
  ...
  *
  * Provides low-level access to libcurl functions for making HTTP requests. Requires libcurl 7.56.0
  * or newer to be installed on the system.
  *
  * @see
...
object CurlCallbacks:
  ...
  private def appendToBuffer(buffer: Ptr[ResponseBuffer], data: Ptr[Byte], size: Long): Unit =
    ...
    if newSize > currentCap then
      val newCap  = math.max(newSize * 2, 1024L)
      val newData = stdlib.malloc(newCap.toCSize)
      val oldData = buffer._1

      if currentSize > 0 && oldData != null then
