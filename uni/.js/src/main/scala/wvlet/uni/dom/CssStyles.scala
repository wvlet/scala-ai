...
case class StyleProperty(name: String):
  /**
    * Create a style attribute with the given value.
    */
  def :=(value: String): DomAttribute = DomAttribute("style", s"${name}: ${value};", append = true)

/**
  * Common CSS style properties for type-safe inline styling.
  */
object CssStyles:
  ...
  /**
    ...
    * }}}
    *
    * Note: Use `styleAttr` if you prefer the longer name, or individual style properties like
