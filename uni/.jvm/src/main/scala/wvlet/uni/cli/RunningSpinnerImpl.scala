...
/**
  * JVM-specific running spinner with animation.
  */
private class JvmRunningSpinner(config: Spinner) extends RunningSpinner:
  private val currentText                         = new AtomicReference[String](config.text)
  private val running                             = new AtomicBoolean(true)
  private val frameIndex                          = new AtomicInteger(0)
  private val frames                              = config.style.frames
  private val scheduler: ScheduledExecutorService = Executors.newSingleThreadScheduledExecutor(r =>
    val t = new Thread(r, "spinner")
    t.setDaemon(true)
    t
  )

  private var scheduledTask: ScheduledFuture[?] = uninitialized
