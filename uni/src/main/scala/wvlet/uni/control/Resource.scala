...
trait Resource[A] extends AutoCloseable:
  ...
  /**
    * Use the resource within the limited scope. After existing the scope, the resource will be
    * closed
    * @param body
    * @tparam U
    * @return
  ...
  /**
    ...
    */
  def wrapFuture[U](body: A => Future[U])(implicit ec: ExecutionContext): Future[U] = Future
    .apply(get)
    .flatMap { (a: A) =>
      body(a)
    }
...
object Resource:
  ...
  /**
    ...
    * @return
    */
  def newTempFile(name: String, suffix: String = ".tmp", dir: String = "target"): Resource[File] =
    newResource[File](
      resource =
        new File(dir)
  ...
  def newResource[R](
    ...
    new Resource[R]:
      ...
      override def close(): Unit = onClose(resource)
