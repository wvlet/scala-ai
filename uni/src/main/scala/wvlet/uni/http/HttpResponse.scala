...
case class HttpResponse(
  ...
  def addHeader(name: String, value: String): HttpResponse = copy(headers =
    headers.add(name, value)
  )

  def setHeader(name: String, value: String): HttpResponse = copy(headers =
    headers.set(name, value)
  )

  def removeHeader(name: String): HttpResponse = copy(headers = headers.remove(name))

  def withTextContent(text: String): HttpResponse = copy(content = HttpContent.text(text))

  def withJsonContent(json: JSONValue): HttpResponse = copy(content = HttpContent.json(json))

  def withJsonContent(json: String): HttpResponse = copy(content = HttpContent.json(json))

  def withBytesContent(bytes: Array[Byte]): HttpResponse = copy(content = HttpContent.bytes(bytes))

  def withHtmlContent(html: String): HttpResponse = copy(content = HttpContent.html(html))

  def withContentType(ct: ContentType): HttpResponse = setHeader(
    HttpHeader.ContentType,
    ct.toString
  )

  def withLocation(uri: String): HttpResponse = setHeader(HttpHeader.Location, uri)

end HttpResponse

object HttpResponse:
  def apply(status: HttpStatus): HttpResponse = HttpResponse(
    status,
    HttpHeaders.empty,
    HttpContent.Empty
  )
  ...
  def redirect(location: String): HttpResponse = HttpResponse(HttpStatus.Found_302).withLocation(
    location
  )

  def movedPermanently(location: String): HttpResponse = HttpResponse(
    HttpStatus.MovedPermanently_301
  ).withLocation(location)

  def seeOther(location: String): HttpResponse = HttpResponse(HttpStatus.SeeOther_303).withLocation(
    location
  )

  def temporaryRedirect(location: String): HttpResponse = HttpResponse(
    HttpStatus.TemporaryRedirect_307
  ).withLocation(location)

  def permanentRedirect(location: String): HttpResponse = HttpResponse(
    HttpStatus.PermanentRedirect_308
  ).withLocation(location)
  ...
  def internalServerError(message: String): HttpResponse = internalServerError.withTextContent(
    message
  )
