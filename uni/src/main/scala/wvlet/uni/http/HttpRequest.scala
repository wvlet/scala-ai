...
case class HttpRequest(
  ...
  def addHeader(name: String, value: String): HttpRequest = copy(headers = headers.add(name, value))

  def setHeader(name: String, value: String): HttpRequest = copy(headers = headers.set(name, value))

  def removeHeader(name: String): HttpRequest = copy(headers = headers.remove(name))
  ...
  def setQueryParam(name: String, value: String): HttpRequest = copy(queryParams =
    queryParams + (name -> Seq(value))
  )

  def setQueryParam(name: String, values: Seq[String]): HttpRequest = copy(queryParams =
    queryParams + (name -> values)
  )

  def withTextContent(text: String): HttpRequest = copy(content = HttpContent.text(text))

  def withJsonContent(json: JSONValue): HttpRequest = copy(content = HttpContent.json(json))

  def withJsonContent(json: String): HttpRequest = copy(content = HttpContent.json(json))

  def withBytesContent(bytes: Array[Byte]): HttpRequest = copy(content = HttpContent.bytes(bytes))

  def withContentType(ct: ContentType): HttpRequest = setHeader(HttpHeader.ContentType, ct.toString)

  def withHost(host: String): HttpRequest = setHeader(HttpHeader.Host, host)

  def withUserAgent(ua: String): HttpRequest = setHeader(HttpHeader.UserAgent, ua)

  def withAccept(accept: String): HttpRequest = setHeader(HttpHeader.Accept, accept)

  def withAuthorization(auth: String): HttpRequest = setHeader(HttpHeader.Authorization, auth)

  def withBearerToken(token: String): HttpRequest = withAuthorization(s"Bearer ${token}")
...
object HttpRequest:
  def apply(method: HttpMethod, uri: String): HttpRequest = HttpRequest(
    method,
    uri,
    HttpHeaders.empty,
    HttpContent.Empty,
