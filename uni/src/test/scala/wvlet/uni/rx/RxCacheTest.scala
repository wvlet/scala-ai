...
class RxCacheTest extends UniTest:
  ...
  test("cache") {
    ...
    v := 2
    evalStream(rx) shouldBe Seq(OnNext(10), OnNext(20))
    ...
    v := 4
    c2.cancel
    events2.result() shouldBe Seq(OnNext(20), OnNext(30), OnNext(40))
  ...
  test("cache.expireAfterWrite") {
    ...
    evalStream(rx) shouldBe Seq(OnNext(30))
  ...
  test("cache for option") {
    ...
    v := Some(2)
    rx.getCurrent shouldBe Some(1)
    evalStream(rx) shouldBe Seq(OnNext(Some(1)), OnNext(Some(2)))
  ...
  test("cache expiration for option") {
    ...
    // Force expiration of the cache
    ticker.advance(1, TimeUnit.MINUTES)
    evalStream(rx) shouldBe Seq(OnNext(Some(2)))
    rx.getCurrent shouldBe Some(2)
  }
