...
class RoundTripTest extends AirSpec with PropertyCheck:
  ...
  test("pack/unpack values") {
    ...
    for v <- list do
      roundtrip(v) {
        OffsetPacker.packValue(_, _)
      } {
        OffsetUnpacker.unpackValue(_)
      }
  }
  ...
  test("report overflow errors") {
    val b1 = Seq(
      BigInteger.valueOf(Byte.MinValue.toLong - 1),
      BigInteger.valueOf(Byte.MaxValue.toLong + 1)
    )
    val b2 = Seq(
      BigInteger.valueOf(Short.MinValue.toLong - 1),
      BigInteger.valueOf(Short.MaxValue.toLong + 1)
    )
    val b3 = Seq(
      BigInteger.valueOf(Int.MinValue.toLong - 1),
      BigInteger.valueOf(Int.MaxValue.toLong + 1)
    )
    ...
    (b1 ++ b2 ++ b3 ++ b4).foreach { b =>
      intercept[IntegerOverflowException] {
        rawRoundtrip(b) {
          OffsetPacker.packBigInteger(_, _)
        } {
          OffsetUnpacker.unpackByte(_)
        }
      }
  ...
  test("support Boolean") {
    roundtrip(true) {
      OffsetPacker.packBoolean(_, _)
    } {
      OffsetUnpacker.unpackBoolean(_)
    }
    roundtrip(false) {
      OffsetPacker.packBoolean(_, _)
    } {
  ...
  test("support Short") {
    ...
    forAll(Gen.chooseNum[Short]((Byte.MaxValue.toShort + 1).toShort, (1 << 8).toShort)) {
      (v: Short) =>
        testShort(v)
    }
  ...
  test("support BigInteger") {
    ...
    forAll { (l: Long) =>
      ...
      roundtrip(v) {
        ...
        OffsetUnpacker.unpackBigInteger(_)
      }
    }
  }
  ...
  test("support String") {
    forAll(arbitrary[String]) { (v: String) => // Generate unicode strings
      roundtrip(v) {
        OffsetPacker.packString(_, _)
      } {
        OffsetUnpacker.unpackString(_)
      }
    }
  }
  ...
  test("support Timestamp") {
    ...
    forAll(posLong, posInt) { (second: Long, nano: Int) =>
      ...
      roundtrip(v) {
        OffsetPacker.packTimestamp(_, _)
      } {
        OffsetUnpacker.unpackTimestamp(_)
      }
    }
  ...
  test("support ArrayHeader") {
    for size <- headerSizes do
      roundtrip(size) {
        OffsetPacker.packArrayHeader(_, _)
      } {
        OffsetUnpacker.unpackArrayHeader(_)
      }
  ...
  test("support MapHeader") {
    for size <- headerSizes do
      roundtrip(size) {
        OffsetPacker.packMapHeader(_, _)
      } {
        OffsetUnpacker.unpackMapHeader(_)
      }
    forAll(sizeGen) { (len: Int) =>
      roundtrip(len) {
        OffsetPacker.packMapHeader(_, _)
  ...
  test("support RawStringHeader") {
    for size <- headerSizes do
      roundtrip(size) {
        OffsetPacker.packRawStringHeader(_, _)
      } {
        OffsetUnpacker.unpackRawStringHeader(_)
      }
    forAll(sizeGen) { (len: Int) =>
      roundtrip(len) {
        OffsetPacker.packRawStringHeader(_, _)
  ...
  test("support BinaryHeader") {
    for size <- headerSizes do
      roundtrip(size) {
        OffsetPacker.packBinaryHeader(_, _)
      } {
        OffsetUnpacker.unpackBinaryHeader(_)
      }
    forAll(sizeGen) { (len: Int) =>
      roundtrip(len) {
        OffsetPacker.packBinaryHeader(_, _)
  ...
  test("support ExtHeader") {
    ...
    for i <- headerSizes do
      roundtrip(ExtTypeHeader(1, i)) {
        OffsetPacker.packExtTypeHeader(_, _)
      } {
        OffsetUnpacker.unpackExtTypeHeader(_)
      }
    forAll(Gen.posNum[Byte], sizeGen) { (v: Byte, len: Int) =>
      roundtrip(ExtTypeHeader(v, len)) {
        OffsetPacker.packExtTypeHeader(_, _)
      } {
        OffsetUnpacker.unpackExtTypeHeader(_)
      }
