...
class RxOpsTest extends AirSpec:
  ...
  test("From RxOption") {
    ...
    test("join") {
      ...
      Rx.some(1)
        .join(Rx.some(2), Rx.some(3))
        .map {
          _ shouldBe (Some(1), Some(2), Some(3))
        }
      Rx.some(1)
        .join(Rx.some(2), Rx.some(3), Rx.some(4))
        .map {
          _ shouldBe (Some(1), Some(2), Some(3), Some(4))
        }
      Rx.some(1)
        .join(Rx.some(2), Rx.some(3), Rx.some(4), Rx.some(5))
        .map {
    ...
    test("zip") {
      ...
      Rx.some(1)
        .zip(Rx.some(2), Rx.some(3))
        .map {
          _ shouldBe (Some(1), Some(2), Some(3))
        }
      Rx.some(1)
        .zip(Rx.some(2), Rx.some(3), Rx.some(4))
        .map {
          _ shouldBe (Some(1), Some(2), Some(3), Some(4))
        }
      Rx.some(1)
        .zip(Rx.some(2), Rx.some(3), Rx.some(4), Rx.some(5))
        .map {
          _ shouldBe (Some(1), Some(2), Some(3), Some(4), Some(5))
        }
    }
