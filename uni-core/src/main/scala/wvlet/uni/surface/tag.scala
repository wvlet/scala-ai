...
/**
  ...
  *
  * Tag instances with arbitrary types. The tags are usually empty `trait` s. Tags have no runtime
  * overhead and are only used at compile-time for additional type safety.
  *
  * For example:
  *
  * {{{
