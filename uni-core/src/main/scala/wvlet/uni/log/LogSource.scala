...
/**
  * A helper to find the source code location for log messages
  */
...
object LogSource:
  def apply()(using code: LogSource) = code

  import scala.quoted.*

  inline implicit def generate: LogSource =
    ${
      generateImpl
    }

  private def generateImpl(using q: Quotes): Expr[LogSource] =
    import q.reflect.*
