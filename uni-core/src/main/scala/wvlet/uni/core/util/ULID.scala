...
import wvlet.uni.core.util.SecureRandom
...
final class ULID(private val ulid: String) extends Ordered[ULID]:
  ...
  /**
    ...
    */
  lazy val epochMillis: Long = CrockfordBase32.decode48bits(ulid.substring(0, 10))
  ...
  override def compare(that: ULID): Int = this.ulid.compareTo(that.ulid)
...
object ULID:
  ...
  /**
    ...
    */
  def useNonSecureRandomULIDGenerator: Unit = setDefaultULIDGenerator(nonSecureRandomULIDGenerator)

  /**
    * Use the default secura-random based ULID generator.
    */
  def useDefaultULIDGenerator: Unit = setDefaultULIDGenerator(defaultULIDGenerator)
  ...
  /**
    ...
    * @return
    */
  def ofMillis(unixTimeMillis: Long): ULID = ULID(_generator.newULIDFromMillis(unixTimeMillis))
  ...
  /**
    ...
    * @return
    */
  def isValid(ulid: String): Boolean = ulid.length == 26 && CrockfordBase32.isValidBase32(ulid)

  /**
    * ULID generator.
    * @param timeSource
    *   a function that returns the current time in milliseconds (e.g.
  ...
  class ULIDGenerator(random: () => Array[Byte]):
    ...
    /**
      ...
      *
      *   1. Reduce the number of Random number generation. SecureRandom is quite slow, so within
      *      the same milliseconds, just incrementing the randomness part will provide better
      *      performance. 2. Generate random in Array[Byte] (10 bytes = 80 bits). Regular Random
      *      uses 48-bit seed, so calling Random.nextInt (32 bits) x 3 is faster, but SecureRandom
      *      has optimization for Array[Byte] generation, which is much faster than calling nextInt
      *      three times. 3. ULIDs are often used in the string value form (e.g., transaction IDs,
      *      object IDs which can be embedded to URLs, etc.). Generating ULID String from the
      *      beginning is ideal. 4. In base32 encoding/decoding, use bit-shift operators as much as
      *      possible to utilize CPU registers and memory cache.
      */
    ...
    private def generateFrom(unixTimeMillis: Long, rand: Array[Byte]): String =
      ...
      val hi =
        ((unixTimeMillis & 0xffffffffffffL) << (64 - 48)) |
          (rand(0) & 0xffL) << 8 |
          (rand(1) & 0xffL)
      val low: Long =
        ((rand(2) & 0xffL) << 56) |
