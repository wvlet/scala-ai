...
case class Count(value: Long, unit: CountUnit) extends Comparable[Count]:
  ...
  def valueOf(unit: CountUnit): Double = value * 1.0 / unit.factor

  def convertTo(unit: CountUnit): Count = Count(value, unit)
...
object Count:
  ...
  def succinct(x: Long): Count = Count(x, ONE).mostSuccinctCount
