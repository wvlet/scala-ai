...
import wvlet.uni.core.util.DataSize.*
import wvlet.uni.core.surface.Surface
import wvlet.uni.core.surface.Zero
...
/**
  ...
  * https://github.com/airlift/units/blob/master/src/main/java/io/airlift/units/DataSize.java for
  * Scala.
  */
case class DataSize(value: Double, unit: DataSizeUnit) extends Comparable[DataSize]:
  ...
  def valueOf(unit: DataSizeUnit): Double = value * (this.unit.factor * 1.0 / unit.factor)
  ...
  def roundTo(unit: DataSizeUnit): Long =
    ...
    checkState(
      ...
      s"${this} is too large to represent in ${unit.unitString} in Long type"
    )
    rounded.toLong
    ...
    dataSizePattern.findFirstMatchIn(dataSizeStr) match
      ...
      case Some(m) =>
        ...
        unitTable.get(unit) match
          case None =>
            throw new IllegalArgumentException(s"Invalid data unit ${unit} in ${dataSizeStr}")
          case Some(u) =>
            DataSize(num, u)
