...
import wvlet.uni.core.log.LogRecord
...
/**
  * Timer trait measures the performance of code blocks. Extend this trait and wrap the code to
  * measure with `time(code_name){ ... }`:
  *
  * {{{
  ...
  * }}}
  *
  * When measuring Scala (Java) code performances, you should take the average of execution times
  * and reorder the code block execution orders, because JVM has JIT compiler, which optimizes the
  * code at runtime. And also cache usage and the running state of the garbage core (GC) affects the
  * code performance. By repeating the executions of the entire or individual blocks with the
  * `repeat` option, you can avoid such pitfalls of benchmarking.
  *
  * @author
  *   leo
  */
trait Timer extends Serializable:
  ...
  protected def block[A](name: String)(f: => A): TimeReport =
    val m =
      contextStack.asScala.lastOption match
        case None =>
  ...
  protected def reportLog(m: TimeReport, logLevel: LogLevel): Unit =
    val l =
      if classOf[Logger].isAssignableFrom(this.getClass) then
...
trait TimeReport extends Ordered[TimeReport]:
  ...
  def containsBlock(name: String) = subMeasure.contains(name)

  def apply(name: String): TimeReport = subMeasure(name)

  def getOrElseUpdate(name: String, t: => TimeReport): TimeReport = subMeasure.getOrElseUpdate(
    name,
    t
  )
  ...
  def compare(that: TimeReport) = this.elapsedSeconds.compareTo(that.elapsedSeconds)
  ...
  def average: Double = s.getElapsedTime / _executionCount

  def elapsedSeconds: Double = s.getElapsedTime
  ...
  def genReportLine: String =
    f"-$name%-15s\ttotal:${toHumanReadableFormat(
        s.getElapsedTime
      )}, count:${executionCount}%,5d, avg:${toHumanReadableFormat(
        average
      )}, min:${toHumanReadableFormat(minInterval)}, median:${toHumanReadableFormat(
        median
      )}, max:${toHumanReadableFormat(maxInterval)}"
...
class StopWatch:
  ...
  /**
    * Reset the stop watch. The subsequent calls to getElapsedTime or getIntervalTime will measure
    * the time interval beginning from this method call.
    */
  def reset: Unit =
  ...
  def reportElapsedTime: String = "%.2f sec." format (getElapsedTime)
