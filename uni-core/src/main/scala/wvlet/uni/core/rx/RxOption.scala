...
import wvlet.uni.core.rx.Rx.CacheOp
import wvlet.uni.core.rx.Rx.FlatMapOp
import wvlet.uni.core.rx.Rx.Join3Op
import wvlet.uni.core.rx.Rx.Join4Op
import wvlet.uni.core.rx.Rx.Join5Op
import wvlet.uni.core.rx.Rx.JoinOp
import wvlet.uni.core.rx.Rx.MapOp
import wvlet.uni.core.rx.Rx.RecoverOp
import wvlet.uni.core.rx.Rx.RecoverWithOp
import wvlet.uni.core.rx.Rx.Zip3Op
import wvlet.uni.core.rx.Rx.Zip4Op
import wvlet.uni.core.rx.Rx.Zip5Op
import wvlet.uni.core.rx.Rx.ZipOp
...
trait RxOption[+A] extends RxOps[Option[A]]:
  ...
  def map[B](f: A => B): RxOption[B] = transformOption {
    case Some(x) =>
      Some(f(x))
    case None =>
      None
  ...
  def flatMap[B](f: A => RxOps[B]): RxOption[B] = RxOptionOp[B](
    transformRx {
      case Some(x) =>
  ...
  def join[B, C, D](b: RxOps[B], c: RxOps[C], d: RxOps[D]): Rx[(Option[A], B, C, D)] = Join4Op(
    this,
    b,
    c,
    d
  ...
  def zip[B, C, D, E](
      ...
      d: RxOps[D],
      e: RxOps[E]
  ): Rx[(Option[A], B, C, D, E)] = Zip5Op(this, b, c, d, e)

  def transform[B](f: Option[A] => B): Rx[B] = MapOp(
    in,
    { (x: Option[A]) =>
      f(x)
    }
  ...
  def transformRx[B](f: Option[A] => RxOps[B]): Rx[B] = in.flatMap(f)

  def transformRxOption[B](f: Option[A] => RxOption[B]): RxOption[B] = RxOptionOp[B](
    FlatMapOp(
      in,
      { (x: Option[A]) =>
        f(x).toRx
  ...
  def transformOption[B](f: Option[A] => Option[B]): RxOption[B] = RxOptionOp[B](
    MapOp(
      in,
      { (x: Option[A]) =>
        f(x)
  ...
  def getOrElse[A1 >: A](default: => A1): Rx[A1] = transform {
    case Some(v) =>
      v
    case None =>
      default
  ...
  def getOrElseRx[A1 >: A](default: => RxOps[A1]): Rx[A1] = transformRx {
    case Some(v) =>
      Rx.single(v.asInstanceOf[A1])
    case None =>
      default
  }

  def orElse[A1 >: A](default: => Option[A1]): RxOption[A1] = transformOption(_.orElse(default))
...
class RxOptionVar[A](variable: RxVar[Option[A]]) extends RxOption[A] with RxVarOps[Option[A]]:
  ...
  override def foreachEvent[U](effect: RxEvent => U): Cancelable = variable.foreachEvent(effect)
  override def update(updater: Option[A] => Option[A], force: Boolean = false): Unit = variable
    .update(updater, force)

  override def setException(e: Throwable): Unit = variable.setException(e)

  override def stop(): Unit = variable.stop()
...
case class RxOptionCacheOp[A](input: RxCache[Option[A]]) extends RxOptionCache[A]:
  ...
  override def withTicker(ticker: Ticker): RxOptionCache[A] = this.copy(input =
    input.withTicker(ticker)
  )
