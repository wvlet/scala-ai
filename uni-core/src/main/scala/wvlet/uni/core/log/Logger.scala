...
class Logger(
    ...
    /**
      * Since java.util.logging.Logger is non-serializable, we need to find the logger instance
      * after deserialization. If wrapped is null, _log method will find or create the logger
      * instance.
      */
    @transient
    private[log] var wrapped: jl.Logger
) extends LoggerBase
    with Serializable:
  ...
  def setLogLevel(l: LogLevel): Unit = _log.setLevel(l.jlLevel)

  def setFormatter(formatter: LogFormatter): Unit = resetHandler(new ConsoleLogHandler(formatter))
  ...
  def getParent: Option[Logger] = Option(wrapped.getParent()).map(x => Logger(x.getName()))

  def addHandler(h: jl.Handler): Unit = _log.addHandler(h)

  def setUseParentHandlers(use: Boolean): Unit = _log.setUseParentHandlers(use)
  ...
  def getHandlers: Seq[jl.Handler] = wrapped.getHandlers.toSeq

  def resetLogLevel: Unit = _log.setLevel(null)
  ...
  /**
    * Suppress warning messages (i.e., setting ERROR log level during the code block). Useful for
    * exception testing
    */
  ...
  /**
    * Suppress the log level around the given Future. After the given Future completes, the log
    * level will be reset to the original level.
    */
  def suppressLogAroundFuture[U](body: => Future[U])(implicit ec: ExecutionContext): Future[U] =
    val prev = getLogLevel
    Future
      .apply(setLogLevel(LogLevel.OFF))
      .flatMap { _ =>
        body
      }
      .transform { case any =>
...
object Logger:
  ...
  /**
    ...
    * @return
    */
  def of[A: ClassTag]: Logger = apply(implicitly[ClassTag[A]].runtimeClass.getName)

  def apply(loggerName: String): Logger =
    val l = loggerCache.getOrElseUpdate(
      loggerName,
  ...
  private def updateLogLevel(l: Logger): Unit = logLevelPatterns
    .collectFirst {
      case (p, level) if p.findFirstIn(l.getName).isDefined =>
        level
    }
    .foreach { level =>
      l.setLogLevel(level)
  ...
  def setDefaultLogLevel(level: LogLevel): Unit = rootLogger.setLogLevel(level)
  ...
  def setDefaultFormatter(formatter: LogFormatter): Unit = synchronized {
    rootLogger.resetHandler(new ConsoleLogHandler(formatter))
  }

  def setDefaultHandler(handler: jl.Handler): Unit = rootLogger.resetHandler(handler)

  def resetDefaultLogLevel: Unit = rootLogger.resetLogLevel

  def clearAllHandlers: Unit = rootLogger.clearAllHandlers
