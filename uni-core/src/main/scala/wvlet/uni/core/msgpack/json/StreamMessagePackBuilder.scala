...
import wvlet.uni.core.msgpack.spi.OffsetPacker
import wvlet.uni.core.msgpack.spi.WriteCursor
...
object StreamMessagePackBuilder:
  ...
  sealed abstract class ParseContext(val offset: Long):
    ...
    def increment: Unit           = elementCount += 1
...
class StreamMessagePackBuilder extends JSONContext[MsgPack] with LogSupport:
  ...
  override def isObjectContext: Boolean = context.isObject
