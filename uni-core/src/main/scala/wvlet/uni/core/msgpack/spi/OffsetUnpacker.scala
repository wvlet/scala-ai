...
import wvlet.uni.core.msgpack.spi.ErrorCode.INVALID_EXT_FORMAT
import wvlet.uni.core.msgpack.spi.ErrorCode.INVALID_TYPE
import wvlet.uni.core.msgpack.spi.ErrorCode.NEVER_USED_FORMAT
import wvlet.uni.core.msgpack.spi.MessageException.*
import wvlet.uni.core.msgpack.spi.MessageFormat.*
...
/**
  * Read a message pack data from a given offset in the buffer. The last read byte length can be
  * checked by calling [[ReadCursor.lastReadLength]] method.
  */
object OffsetUnpacker:
      ...
      mf match
        ...
        case NEVER_USED =>
          throw new MessageException(
            NEVER_USED_FORMAT,
            s"Found 0xC1 (NEVER_USED) byte while skipping a value"
          )
      end match
      count -= 1
  ...
  def skipPayload(cursor: ReadCursor, numBytes: Int): Unit = cursor.skipBytes(numBytes)
  ...
  def unpackByte(cursor: ReadCursor): Byte =
    ...
    b match
      ...
      case Code.UINT16 =>
        ...
        if u16 < 0 || !u16.isValidByte then
          throw overflowU16(u16)
        u16.toByte
      case Code.UINT32 =>
        val u32 = cursor.readInt
        if u32 < 0 || !u32.isValidByte then
          throw overflowU32(u32)
        u32.toByte
      case Code.UINT64 =>
        val u64 = cursor.readLong
        if u64 < 0 || !u64.isValidByte then
  ...
  def unpackShort(cursor: ReadCursor): Short =
    ...
    b match
      ...
      case Code.UINT32 =>
        val u32 = cursor.readInt
        if u32 < 0 || !u32.isValidShort then
          throw overflowU32(u32)
        u32.toShort
      case Code.UINT64 =>
        val u64 = cursor.readLong
        if u64 < 0 || !u64.isValidShort then
          throw overflowU64(u64)
        u64.toShort
      case Code.INT8 =>
        val i8 = cursor.readByte
      ...
      case Code.INT32 =>
        val i32 = cursor.readInt
        if !i32.isValidShort then
          throw overflowI32(i32)
        i32.toShort
      case Code.INT64 =>
        val i64 = cursor.readLong
        if !i64.isValidShort then
          throw overflowI64(i64)
        i64.toShort
      case _ =>
        cursor.reverseCursor
  ...
  def unpackTimestamp(extTypeHeader: ExtTypeHeader, cursor: ReadCursor): Instant =
    ...
    val instant =
      extTypeHeader.byteLength match
        case 4 =>
  ...
  /**
    * Read a payload of the given length from the given buffer[position], and write the result to
  ...
  private[spi] def unexpected(expectedCode: ValueType, actual: Byte): Nothing =
    ...
    if f == MessageFormat.NEVER_USED then
      throw new MessageException(
        NEVER_USED_FORMAT,
        s"Expected ${expectedCode}, but found 0xC1 (NEVER_USED) byte"
      )
    else
      ...
      throw new MessageException(
        INVALID_TYPE,
        f"Expected ${expectedCode}, but got ${typeName} (${actual}%02x)"
      )
