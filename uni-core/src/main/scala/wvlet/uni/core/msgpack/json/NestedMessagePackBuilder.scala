...
class NestedMessagePackBuilder extends JSONContext[Seq[MsgPack]] with LogSupport:
  ...
  override def addBoolean(s: JSONSource, v: Boolean, start: Int, end: Int): Unit = packer
    .packBoolean(v)

  override def singleContext(s: JSONSource, start: Int): JSONContext[Seq[MsgPack]] =
...
end NestedMessagePackBuilder

/**
  * A JSON parse context implementation for remembering how many elements are added to an object or
  * array
  */
class LocalStructureContext extends NestedMessagePackBuilder:
  self =>
  ...
  override def addBoolean(s: JSONSource, v: Boolean, start: Int, end: Int): Unit =
    elementCount += 1
    super.addBoolean(s, v, start, end)
