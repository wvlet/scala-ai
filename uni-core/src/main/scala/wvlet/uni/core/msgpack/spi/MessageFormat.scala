...
object MessageFormat:
  ...
  /**
    * Converting a byte value into MessageFormat. For faster performance, use [[valueOf(byte)]]
    * instead.
    *
    * @param b
    *   MessageFormat of the given byte
  ...
  private[spi] def toMessageFormat(b: Byte): MessageFormat =
    if Code.isPosFixInt(b) then
      MessageFormat.POSFIXINT
    else if Code.isNegFixInt(b) then
      MessageFormat.NEGFIXINT
    else if Code.isFixStr(b) then
      MessageFormat.FIXSTR
    else if Code.isFixedArray(b) then
      MessageFormat.FIXARRAY
    else if Code.isFixedMap(b) then
