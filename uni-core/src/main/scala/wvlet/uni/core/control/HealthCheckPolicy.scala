...
import wvlet.uni.core.control.util.ExponentialMovingAverage
...
object HealthCheckPolicy extends LogSupport:
  ...
  /**
    ...
    */
  def alwaysHealthy: HealthCheckPolicy =
  ...
  def markDeadOnConsecutiveFailures(numFailures: Int): HealthCheckPolicy =
    new HealthCheckPolicy:
      ...
      override def recordSuccess(): Unit = consecutiveFailures.set(0)

      /**
        * Called when request is failed. Returns delay
        */
      override def recordFailure(): Unit = consecutiveFailures.incrementAndGet()
  ...
  def markDeadOnRecentFailureRate(failureRate: Double, timeWindowMillis: Long): HealthCheckPolicy =
    new HealthCheckPolicy:
      ...
      /**
        * Called when a request succeeds
        */
      override def recordSuccess(): Unit = failureRateEMA.update(System.currentTimeMillis(), 0)

      /**
        * Called when request is failed.
        */
      override def recordFailure(): Unit = failureRateEMA.update(System.currentTimeMillis(), 1)

      /**
        * Called when the target service is recovered()
        */
      override def recovered(): Unit = failureRateEMA.reset()
  ...
  def markDeadOnFailureThreshold(numFailures: Int, numExecutions: Int) =
    ...
    new HealthCheckPolicy:
      ...
      override def recovered(): Unit = executionCount = 0

    end new

  end markDeadOnFailureThreshold

end HealthCheckPolicy
