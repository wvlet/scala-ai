...
import wvlet.uni.core.control.Retry.ExtraWait
...
object ResultClass:
  ...
  case class Failed(
      isRetryable: Boolean,
      cause: Throwable,
      extraWait: ExtraWait = Retry.noExtraWait
  ) extends ResultClass:
    def withExtraWaitMillis(extraWaitMillis: Int): Failed = this.copy(extraWait =
      ExtraWait(maxExtraWaitMillis = extraWaitMillis)
    )

    def withExtraWaitFactor(factor: Double, maxExtraWaitMillis: Int = 5000): Failed = this.copy(
      extraWait = ExtraWait(maxExtraWaitMillis = maxExtraWaitMillis, factor = factor)
    )
