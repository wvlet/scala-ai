...
object Control:
  ...
  def withResources[R1 <: AutoCloseable, R2 <: AutoCloseable, U](resource1: R1, resource2: R2)(
    ...
    try body(resource1, resource2)
    finally closeResources(resource1, resource2)
  ...
  def withResourceAsync[R <: AutoCloseable, U](
      resource: R
  )(body: R => Future[U])(implicit sc: ExecutionContext): Future[U] = Future
    .apply(resource)
    .flatMap(r => body(r))
    .transform { case any =>
  ...
  /**
    * Safely close the resource. If the resource is closed successfully, return the preceding
    * result, if closing the resource is failed, combine the previous exception (if exists) and the
    * resource closing exception together.
    */
