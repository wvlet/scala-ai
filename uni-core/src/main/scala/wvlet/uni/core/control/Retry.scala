...
import wvlet.uni.core.control.ResultClass.Failed
import wvlet.uni.core.log.LogSupport
import wvlet.uni.core.rx.Rx
import wvlet.uni.core.util.ThreadUtil
...
object Retry extends LogSupport:
  ...
  ): RetryContext = defaultRetryContext
  ...
  def withBoundedBackoff(
    ...
    val N =
      math.log(1 - (maxTotalWaitMillis * (1 - multiplier) / initialIntervalMillis)) /
        math.log(multiplier)
  ...
  case class MaxRetryException(retryContext: RetryContext)
      extends Exception(
        s"Reached the max retry count ${retryContext.retryCount}/${retryContext
            .maxRetry}: ${retryContext.lastError.getMessage}",
  ...
  private def REPORT_RETRY_COUNT: RetryContext => Unit =
    (ctx: RetryContext) =>
      warn(
        f"[${ctx.retryCount}/${ctx.maxRetry}] Execution failed: ${ctx
            .lastError
            .getMessage}. Retrying in ${ctx.nextWaitMillis / 1000.0}%.2f sec."
      )
  ...
  case class ExtraWait(maxExtraWaitMillis: Int = 0, factor: Double = 0.0):
    ...
    def hasNoWait: Boolean = maxExtraWaitMillis == 0 && factor == 0.0
  ...
  case class RetryContext(
    ...
    def init(context: Option[Any] = None): RetryContext = this.copy(
      context = context,
      lastError = NOT_STARTED,
      retryCount = 0,
    ...
    def canContinue: Boolean = retryCount < maxRetry
    ...
    def withRetryWaitStrategy(newRetryWaitStrategy: RetryPolicy): RetryContext = this.copy(
      retryWaitStrategy = newRetryWaitStrategy
    )

    def withMaxRetry(newMaxRetry: Int): RetryContext = this.copy(maxRetry = newMaxRetry)

    def noRetry: RetryContext = this.copy(maxRetry = 0)
    ...
    def withResultClassifier[U](newResultClassifier: U => ResultClass): RetryContext = this.copy(
      resultClassifier = newResultClassifier.asInstanceOf[Any => ResultClass]
    )

    /**
      * Set a detailed error handler upon Exception. If the given exception is not retryable, just
      * rethrow the exception. Otherwise, consume the exception.
      */
    def withErrorClassifier(errorClassifier: Throwable => ResultClass.Failed): RetryContext = this
      .copy(errorClassifier = errorClassifier)

    def beforeRetry[U](handler: RetryContext => U): RetryContext = this.copy(beforeRetryAction =
      handler
    )
    ...
    /**
      ...
      */
    def noRetryLogging: RetryContext = this.copy(beforeRetryAction = (x: RetryContext) => ())
    ...
    def run[A](body: => A): A = runInternal(None)(body)

    def runWithContext[A](
        context: Any,
        circuitBreaker: CircuitBreaker = CircuitBreaker.alwaysClosed
    )(body: => A): A = runInternal(Option(context), circuitBreaker)(body)
  ...
  trait RetryPolicy:
    ...
    def updateBaseWait(waitMillis: Int): Int = math
      .round(waitMillis * retryPolicyConfig.multiplier)
      .toInt
      .min(retryPolicyConfig.maxIntervalMillis)

    def nextWait(baseWaitMillis: Int): Int

  class ExponentialBackOff(val retryPolicyConfig: RetryPolicyConfig) extends RetryPolicy:
    override def nextWait(baseWaitMillis: Int): Int = baseWaitMillis

  class Jitter(val retryPolicyConfig: RetryPolicyConfig, rand: Random = new Random())
      extends RetryPolicy:
    override def nextWait(baseWaitMillis: Int): Int = (baseWaitMillis.toDouble * rand.nextDouble())
      .round
      .toInt
