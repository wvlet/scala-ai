...
import wvlet.uni.core
import wvlet.uni.core.surface
...
private[surface] object CompileTimeSurfaceFactory:
  ...
  type SurfaceMatcher = PartialFunction[Type[?], Expr[Surface]]
...
private[surface] class CompileTimeSurfaceFactory[Q <: Quotes](using quotes: Q):
  ...
  def surfaceOf(tpe: Type[?]): Expr[Surface] = surfaceOf(TypeRepr.of(using tpe))
      ...
      val generator = factory.andThen { expr =>
        if !lazySurface.contains(t) then
          ...
          '{
            ...
            then
              core.surface.Surface.surfaceCache +=
                ${
                  key
                } ->
                  ${
  ...
  private def hasSecretAnnotation(s: Symbol): Boolean =
    val t = TypeRepr.of[wvlet.uni.core.surface.secret]
    s.hasAnnotation(t.typeSymbol)

  private def hasRequiredAnnotation(s: Symbol): Boolean =
    val t = TypeRepr.of[wvlet.uni.core.surface.required]
    s.hasAnnotation(t.typeSymbol)
  ...
  def methodsOf(t: Type[?]): Expr[Seq[MethodSurface]] = methodsOf(TypeRepr.of(using t))
  ...
  private def methodsOf(t: TypeRepr): Expr[Seq[MethodSurface]] =
    ...
    /**
      ...
      * ClassMethodSurface( .... ) }}
      */
    val expr = Block(surfaceDefs, methodsOfInternal(t).asTerm).asExprOf[Seq[MethodSurface]]
