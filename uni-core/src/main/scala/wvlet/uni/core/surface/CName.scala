...
/**
  * Utility for managing names written in different spellings. For example, variable name
  * <i>localAddress</i> can be written as "local address", "local_address", etc.
  *
  ...
  */
object CName:
  private val cnameTable = HashMap[String, CName]()

  def apply(name: String): CName = cnameTable.getOrElseUpdate(
    name,
    new CName(toCanonicalName(name), toNaturalName(name))
  )

  private val paramNameReplacePattern = Pattern.compile("[\\s-_]");
  private val canonicalNameTable      = HashMap[String, String]()
  private val naturalNameTable        = HashMap[String, String]()
  ...
  def toNaturalName(varName: String): String =
    if varName == null then
      ...
      def translate(varName: String) =
        def wikiNameComponents: List[String] = findWikiNameComponent(0)
...
/**
  * Canonical name. This name is used as a common name of wording variants (e.g., difference of
  * capital letter usage, hyphenation, etc.)
  *
  * @param canonicalName
  * @param naturalName
  */
