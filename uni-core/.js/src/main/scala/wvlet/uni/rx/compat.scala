...
object compat:
  def defaultExecutionContext: scala.concurrent.ExecutionContext =
    MacrotaskExecutor.Implicits.global
      ...
      override def schedule[U](millis: Long)(body: Long => U): Unit =
        val handle =
          scala
            ...
            .js
            .timers
            .setInterval(millis.toDouble) {
              val currentTimeMillis = System.currentTimeMillis()
              try body(currentTimeMillis - lastTimeMillis)
              finally lastTimeMillis = currentTimeMillis
            }
        intervalHandle = Some(handle)
      override def cancel: Unit = intervalHandle.foreach { handle =>
