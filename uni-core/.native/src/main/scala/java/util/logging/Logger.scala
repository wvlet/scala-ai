...
class Logger(parent: Option[Logger], name: String):
  ...
  def log(level: Level, msg: String): Unit = log(LogRecord(level, msg))

  def log(record: LogRecord): Unit =
    if isLoggable(record.getLevel()) then
      if record.getLoggerName() == null then
  ...
  private[logging] def publish(record: LogRecord): Unit =
    if useParentHandlers && parent.nonEmpty then
  ...
  def getParent(): Logger = parent.getOrElse(null)

  def getLevel(): Level = level.orElse(parent.map(_.getLevel())).getOrElse(Level.INFO)

  def setLevel(newLevel: Level): Unit = level = Option(newLevel)

  def resetLogLevel(): Unit = level = None

  def setUseParentHandlers(useParentHandlers: Boolean): Unit =
    this.useParentHandlers = useParentHandlers

  def addHandler(h: Handler): Unit = handlers = h :: handlers

  def removeHandler(h: Handler): Unit = handlers = handlers.filter(_ != h)
...
/**
  * Stub implementation of java.util.logging.Formatter for Scala Native
  */
abstract class Formatter:
  def format(record: LogRecord): String

/**
  * Stub implementation of java.util.logging.LogRecord for Scala Native
  */
